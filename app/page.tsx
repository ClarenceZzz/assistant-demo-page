...
type ComposerProps = {
  ...
  onSend: () => void;
  chips: Record<ChipOption, boolean>;
...
const createMessage = (role: MessageRole, content: string, options: MessageOptions = {}): ConversationMessage => ({
      ...
      {messages.map((message) =>
        message.role === "assistant" ? (
          ...
          <div key={message.id} className={`${styles.messageGroup} ${styles.messageGroupUser}`}>
            ...
            <span className={styles.messageTimestamp}>{formatRelativeTime(message.createdAt)}</span>
          </div>
        )
      )}
      {isWaiting && (
        <div className={styles.pendingRow}>
          <Image src="/assets/assistant-status-thinking.svg" alt="" width={16} height={16} aria-hidden="true" />
          <span>正在思考…</span>
        </div>
      )}
    </div>
    ...
    <section className={styles.voiceCard} aria-labelledby="voice-title">
      <div className={styles.voiceHeader}>
        ...
        >
          <svg viewBox="0 0 34 34" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
            ...
            <rect x="0.5" y="0.5" width="33" height="33" rx="16.5" stroke="white" strokeOpacity="0.04" fill="none" />
            <path
      ...
      <div className={styles.voiceFooter}>
        <div className={styles.voiceChips}>
          <button
            type="button"
            className={`${styles.voiceChip} ${chips.deep ? styles.chipPrimary : styles.chipSecondary}`}
            onClick={() => onChipToggle("deep")}
            aria-pressed={chips.deep}
          >
            <svg
          ...
          <button
            type="button"
            className={`${styles.voiceChip} ${chips.search ? styles.chipPrimary : styles.chipSecondary}`}
            onClick={() => onChipToggle("search")}
            aria-pressed={chips.search}
          >
            <svg
              className={styles.voiceChipIcon}
              viewBox="0 0 16 16"
        ...
        <div className={styles.voiceModelWrapper}>
          <select
            ...
            value={selectedModel}
            onChange={(event) => onModelChange(event.target.value as typeof MODELS[number])}
            aria-label="选择模型"
          >
            {MODELS.map((model) => (
